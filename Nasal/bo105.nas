...
var min = func(a, b) a < b ? a : b;
 ...
 setprop("/instrumentation/cwp/PITOT",1);
 setprop("/instrumentation/cwp/ENGP", 1);
 setprop("/instrumentation/cwp/MGBP", 1);
 setprop("/instrumentation/cwp/MGBT", 0);
 setprop("/instrumentation/cwp/GEN",  1);
 setprop("/instrumentation/cwp/ALTNR",1);
#setprop("/instrumentation/cwp/BAT",  0); #not used in the sim
 setprop("/instrumentation/cwp/NAV",  1);
 setprop("/instrumentation/cwp/FUEL", 0);
#setprop("/instrumentation/cwp/FFILT",0); #not used in the sim
 ...
 setprop("/engines/engine/starting/switch-angle",45);
 setprop("/engines/engine/starting/alarm",0); #not used at the moment (11 Aug 16) 
 setprop("/engines/engine/starting/start",0); #- will be part of a later mod 
 setprop("/engines/engine/starting/inj",0);
 setprop("/engines/engine/starting/stop",0);
...
var engines = {
	...
	update: func(dt) {
		...
		if (n2max > 0.67) { # 0.63?
			...
			if(getprop("/engines/engine/n1-rpm") >= 25000){setprop("/instrumentation/cwp/GEN", 0);}
		else{                                          setprop("/instrumentation/cwp/GEN", 1);}
		
		if(getprop("/engines/engine/n1-rpm") >= 26000){setprop("/instrumentation/cwp/PITOT", 0);}
		else{                                          setprop("/instrumentation/cwp/PITOT", 1);}
		
		if(getprop("/engines/engine/n1-rpm") >= 27000){setprop("/instrumentation/cwp/ENGP", 0);}
		else{                                          setprop("/instrumentation/cwp/ENGP", 1);}		
		
		if(getprop("/engines/engine/n1-rpm") >= 30000){setprop("/instrumentation/cwp/ALTNR", 0);}
		else{                                          setprop("/instrumentation/cwp/ALTNR", 1);}
		
		if(getprop("/engines/engine/n1-rpm") >= 31000){setprop("/instrumentation/cwp/NAV", 0);}
		else{                                          setprop("/instrumentation/cwp/NAV", 1);}
		
		if(getprop("/rotors/main/rpm") >= 280){setprop("/instrumentation/cwp/MGBP", 0);}
		else{                                  setprop("/instrumentation/cwp/MGBP", 1);}
		...
		setprop("/instrumentation/heading-indicator/heading-bug-deg", 
			getprop("instrumentation/gps/wp/wp[1]/bearing-mag-deg") );	
...
var procedure = {
	...
	process: func(id) {
		...
		# startup
		if(0 >= me.stage){setprop("/engines/engine/starting/switch-angle",45);}  # Warty Mod -->
		if (me.stage == 1) {
...
var vibration = { # and noise ...
	...
	update: func(dt) {
		...
		if (airspeed > 167) { # overspeed vibration
